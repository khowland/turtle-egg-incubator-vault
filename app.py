...
<style>
    ...
    /* SIDEBAR - FORCED WHITE TEXT (FIXES DARK-ON-DARK) */
...
if menu == "📊 DASHBOARD":
    ...
    try:
        ...
        # ALERTS: FIXED SCHEMA - Using bin.harvest_date
    ...
    with st.form("intake_form_v5", clear_on_submit=True):
        ...
        if st.form_submit_button("SAVE INTAKE & REGISTER EGGS"):
            try:
                ...
                m_id = m_res.data[0]['mother_id'] if m_res.data else supabase.table("mother").insert({"mother_name": mother_name, "species_id": spec_map[species_name]}).execute().data[0]['mother_id']
