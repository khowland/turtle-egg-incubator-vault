...
<style>
    ...
    [data-testid="stSidebar"] [data-testid="stWidgetLabel"] p { color: #FFFFFF !important; font-size: 1.1rem !important; }

    /* Primary Button - EMERALD (FIXES WHITE-ON-WHITE) */
    div.stButton > button[kind="primaryFormSubmit"], div.stButton > button {
...
with st.sidebar:
    ...
    st.v_spacer(height=200)
    st.caption(f"Core Version: {VERSION}")
...
if menu == "📊 DASHBOARD":
    ...
    try:
        ...
        with c1: st.markdown(f'<div class="glass-card"><b>ACTIVE EGGS</b><br><span style="font-size:2.5rem; font-weight:bold;">{active}</span></div>', unsafe_allow_html=True)
        with c2: st.markdown(f'<div class="glass-card"><b>PIPPING</b><br><span style="font-size:2.5rem; font-weight:bold;">{pip}</span></div>', unsafe_allow_html=True)
        with c3: st.markdown(f'<div class="glass-card"><b>SYNC</b><br><span style="font-size:2.5rem; font-weight:bold;">100%</span></div>', unsafe_allow_html=True)
        ...
        if res:
            for egg in res:
                ...
                if h_date and h_date < limit:
                    ...
                    alerts += 1
        if alerts == 0: st.success("✓ Neural scan complete. All specimens stable.")

    except Exception as e: st.error(f"Neural Uplink Error: {e}")

elif menu == "🐣 NEW INTAKE":
    st.markdown("<h1>New Intake</h1>", unsafe_allow_html=True)
    st.markdown('<div class="glass-card">', unsafe_allow_html=True)
    with st.form("intake_v47", clear_on_submit=True):
        ...
        if st.form_submit_button("SAVE INTAKE & REGISTER EGGS"):
            try:
                ...
                st.balloons()
                st.success(f"Vault Synchronized: {qty} eggs registered.")
