...
def test_generate_mid_season_data():
    ...
    for i, (code, name) in enumerate(SPECIES_LIST):
        ...
        at_intake.selectbox(key="intake_species").set_value(label).run()
        at_intake.text_input(key="intake_name").set_value(case_id).run()
        at_intake.text_input(key="intake_finder").set_value(finder).run()
        at_intake.selectbox(key="intake_condition").set_value("Alive").run()
    ...
    for bid in bin_ids:
        ...
        at_obs.multiselect(key="obs_workbench").set_value([bid]).run()
        at_obs.selectbox(key="Current Bin Focus").set_value(bid).run()
        
        if not at_obs.session_state.env_gate_synced.get(bid):
            at_obs.number_input(key="wt_gate").set_value(160.0).run()
            at_obs.number_input(key="obs_temp").set_value(82.5).run()
            at_obs.button(key="obs_env_save").click().run()
        
        # 2. Add Observations for a subset of eggs
        eggs_res = get_supabase().table("egg").select("egg_id, current_stage").eq("bin_id", bid).execute()
        eggs = eggs_res.data
        ...
        at_obs.selectbox(key="matrix_stage").set_value(new_stage).run()
        at_obs.selectbox(key="matrix_status").set_value("Active" if new_stage != "S6" else "Transferred").run()
        at_obs.selectbox(key="matrix_chalking").set_value("Medium").run()
        
        at_obs.selectbox(key="matrix_molding").set_value(1).run()
        at_obs.selectbox(key="matrix_leaking").set_value(0).run()
        at_obs.selectbox(key="matrix_denting").set_value(0).run()
        ...
        if len(eggs) > 5:
            ...
            at_obs.run()
            at_obs.selectbox(key="matrix_status").set_value("Dead").run()
            at_obs.button(key="obs_matrix_save").click().run(timeout=30)
