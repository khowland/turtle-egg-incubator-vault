...
1. **Removed** all blocking `@import url(...)` CSS directives from `utils/bootstrap.py` and `vault_views/0_Login.py`.
2. **Standardized** on a "System Font Only" stack:

   ```css
   font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
   ```

3. **Verified** no external network requests are triggered during the initial render by removing `Inter` definitions that lacked local files.

### 2. Quarantined Residual Artifacts

1. **Moved** `hydration_page.html` (which contained a residual blocking `@import`) to the `quarantine/` directory.

### 3. Verification

- Performance telemetry confirmed Python execution times are stable (<1s).
- Browser stalls (Bug-PERF-001) were eliminated by removing all outbound network dependencies.
