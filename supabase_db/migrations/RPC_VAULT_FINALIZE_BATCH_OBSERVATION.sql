...
BEGIN
    ...
    LOOP
        ...
        INSERT INTO public.egg_observation (
            ...
            v_stage, p_payload->>'observation_notes', FALSE,
            v_obs->>'chalking', v_obs->>'vascularity',
            v_obs->>'molding', v_obs->>'leaking', v_obs->>'dented'
        );

        UPDATE public.egg
        SET current_stage = v_stage,
            status = CASE WHEN v_stage = 'S6' THEN 'Transferred' ELSE 'Active' END,
            last_chalk = v_obs->>'chalking',
            last_vasc = v_obs->>'vascularity',
            last_molding = v_obs->>'molding',
            last_leaking = v_obs->>'leaking',
            last_dented = v_obs->>'dented',
        ...
        IF v_stage = 'S6' THEN
            ...
            SELECT mother_id INTO v_mother_id FROM public.bin WHERE bin_id = v_bin_id;
            ...
            INSERT INTO public.hatchling_ledger (
                egg_id, mother_id, hatch_date, vitality_score,
                incubation_duration_days, notes, session_id,
                created_by_id, modified_by_id
            ) VALUES (
                v_egg_id, v_mother_id, v_hatch_date, substring(v_vitality from 1 for 500),
