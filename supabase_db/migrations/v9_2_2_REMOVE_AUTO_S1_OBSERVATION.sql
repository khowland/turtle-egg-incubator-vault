-- ============================================================================
-- v9.2.2: Remove auto-S1 egg_observation from vault_finalize_intake
-- Root Cause: Auto-creating S1 egg_observation during intake makes all new eggs
-- appear "already observed" on Observations page. START button filters to
-- unobserved eggs only, finds none, selected_eggs stays empty, Property Matrix
-- condition (if selected_real_ids:) is False → never renders → tests timeout.
-- Fix: Remove egg_observation INSERT from the egg creation loop. Eggs already
-- have current_stage=S1 as default state. egg_observation should only be
-- created by user action via Property Matrix, not during intake.
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    FOR v_i IN 1..v_egg_count LOOP
      ...
      -- v9.2.2: REMOVED auto-S1 egg_observation INSERT (was causing START button to see all eggs as observed)
    END LOOP;
  END LOOP;
