-- Rename carapace_length_mm to mother_weight_g
ALTER TABLE public.intake RENAME COLUMN carapace_length_mm TO mother_weight_g;

-- Update the vault_finalize_intake RPC to accept mother_weight_g instead of carapace_length_mm
...
BEGIN
  ...
  PERFORM intake_count FROM public.species WHERE species_id = v_species_id FOR UPDATE;
  v_next_intake := (SELECT intake_count FROM public.species WHERE species_id = v_species_id);
  ...
  INSERT INTO public.intake (
    ...
    intake_condition, days_in_care, extraction_method, discovery_location, mother_weight_g,
  ...
  LOOP
    ...
    INSERT INTO public.bin (
      ...
      v_bin_id, v_intake_id, v_notes, v_egg_count, (v_bin->>'bin_weight_g')::numeric,
      (v_bin->>'incubator_temp_c')::numeric, v_bin->>'substrate', v_bin->>'shelf_location',
      v_session_id, v_observer_id, v_observer_id
    );
    ...
    INSERT INTO public.bin_observation (
      ...
      v_session_id, v_bin_id, v_observer_id, (v_bin->>'bin_weight_g')::numeric,
  ...
  RETURN jsonb_build_object('intake_id', v_intake_id, 'first_bin_id', v_first_bin);
END;
$function$;
